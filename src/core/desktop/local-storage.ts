...
function parseLogFile(data: Uint8Array): KvEntry[] {
  ...
  while (offset < data.length) {
    ...
    const payload = data.slice(offset + HEADER_SIZE, offset + HEADER_SIZE + length);
...
function buildFullBatchRecord(entries: Map<string, Uint8Array>): Uint8Array {
  ...
  for (const [key, value] of entries) {
    ...
    const entry = new Uint8Array(1 + keyLenV.length + keyBytes.length + valLenV.length + value.length);
    let p = 0;
    entry[p++] = BATCH_PUT;
    entry.set(keyLenV, p); p += keyLenV.length;
    entry.set(keyBytes, p); p += keyBytes.length;
    entry.set(valLenV, p); p += valLenV.length;
    entry.set(value, p);
    parts.push(entry);
  }
 ...
 */
function buildManifest(logNumber: number, nextFileNumber: number, lastSequence: number): Uint8Array {
